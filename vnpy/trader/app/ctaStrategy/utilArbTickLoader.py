...
class UtilArbTickLoader(object):
    ...
    def loadArbTicks(self, mainPath, dtDate, leg1Symbol, leg2Symbol):
        ...
        self.writeCtaLog('加载日期:{0}\{1}的价差tick'.format(mainPath, dtDate))
        cachefilename = '{0}_{1}_{2}_{3}_{4}'.\
        ...
        if len(arbTicks) < 1:
            ...
            leg1File = self.ticksFolder + '\\{0}\\{1}\\{2}\\{3}\\{4}.txt' \
            ...
            leg2File = self.ticksFolder + '\\{0}\\{1}\\{2}\\{3}\\{4}.txt' \
            ...
            for row in reader:
                ...
                if dtStr in leg2Ticks:
                    self.writeCtaError('日内数据重复，异常,数据时间为:{0}'.format(dtStr))
                else:
    ...
    def loadDataFrame(self, mainPath, dtDate, leg1Symbol, leg2Symbol):
        ...
        self.writeCtaLog('加载日期:{0}\{1}的价差tick dataframe'.format(mainPath, dtDate))
        cachefilename = '{0}_{1}_{2}_{3}_{4}'. \
            format(self.symbol, leg1Symbol, leg2Symbol, mainPath, dtDate.strftime('%Y%m%d'))
    ...
    def loadDataFrame2(self, mainPath, beginDate, endDate, leg1Symbol, leg2Symbol):
        ...
        for i in range(0, dayIntervals):
            ...
            self.output('取数据日期:{0}'.format(getDate))
            ...
            if df1 is not None and df is None:
                df = copy.copy(df1)
                self.output('数据{0}行'.format(len(df)))
            elif df1 is not None and df is not None:
                df = pandas.concat([df,df1], ignore_index=True)
                self.output('数据增加{0}行,共{1}行'.format(len(df1),len(df)))
            ...
            if df2 is not None and df is None:
                ...
                self.output('数据增加{0}行,共{1}行'.format(len(df2), len(df)))
